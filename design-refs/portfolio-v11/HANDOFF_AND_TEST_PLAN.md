...
| 4 — Fixes by severity | **Done** — all findings but one fixed, deployed to dev and live-verified: PV11-002 (capital call fund/amount was silently lost, same bridge-override bug class as PV11-004), PV11-004 (LP onboarding — real root cause was `portfolio-runtime-live-bridge.inc.js` shadowing the first fix attempt), PV11-005 (new "New distribution" wizard built from scratch on the Funds page, wired to the previously-unused `lpFeesApi.declareDistribution`), PV11-006 (deal-detail hero status), PV11-007 (backend `authorize()` gap, repo-wide), PV11-008 (board-review multipart bug), PV11-010 (Deal Flow's filters — Dashboard's half still needs `renderDashboard` wiring, a smaller follow-up), PV11-011 (Period Close's dead buttons). Only PV11-009 left open — no code fix, flagged for a product decision. **Key discovery, worth knowing for the next module's sweep too:** `portfolio-runtime-live-bridge.inc.js` independently overrides several base runtime functions for the live-data path (confirmed for `submitLP`, `submitCapitalCall`) — a fix to the base `matanho-portfolio-runtime.js` function of the same name can be silently shadowed; always check this file first. Also: `node --check` does not catch every syntax error a real Next.js/webpack build will (caught one during PV11-005's deploy — a plain object property using `${...}` interpolation outside any template literal) — a failed `npm run build` in the deploy log is the real signal, not a clean `node --check`. |
| 5 — Merge & deploy | **Done** — `feature/portfolio-v11-live` merged into `dev` (`6f2a8ac`, frontend), `master` (`a6e8213`, backend), and `prod` (`9c2b036`, backend), all pushed. Production deploy to the NTS stack (`31.220.82.129`, `deploy-nvccz-nts-prod.py`) hit a transient paramiko `EOFError` on the first attempt and a mid-build "Socket exception" on the retry, but both times the script's own completion markers (credentials block, exit code 0) printed. Since the public domains (`matanho.nvccz.com` etc.) are unreachable from this sandbox (browser navigation denied, `curl` DNS resolution blocked — a sandbox/org network restriction, not a prod outage), verified via direct SSH instead: (1) `docker compose ps` showed all 9 prod containers `healthy`, API and every UI container freshly recreated ~10 min prior, matching the retry's timing; (2) `docker exec` into `nvccz-prod-api-1` confirmed the compiled `authorize()` fix (`portfolio_mgr`, 4 matches in `termSheetRoutes.js`) is present in the running container. Both confirm the production deploy succeeded with the intended code. |
