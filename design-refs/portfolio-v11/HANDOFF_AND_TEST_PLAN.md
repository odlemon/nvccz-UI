...
| 3 — Live testing | **In progress** — Wave 1 (capital calls, LP onboarding, distribution creation) done: FINDING-PV11-002 (capital call $ mismatch, logged), FINDING-PV11-004 (LP onboarding broken 100% of the time, root-caused), FINDING-PV11-005 (distribution creation has no UI at all — backend/API client exist, unused; decision: build in Phase 4). STAGE_MAP fallback bug re-checked: `adapters.ts` already maps `TERM_SHEET_CREATED`/`TERM_SHEET_APPROVED`/`PORTFOLIO_COMPANY_CREATED` correctly — no longer reproduces, appears already fixed since the Sept gap-analysis. Wave 2 (deal-lifecycle actions) next. |
