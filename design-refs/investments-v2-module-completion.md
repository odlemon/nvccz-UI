...
**FE closure of this BE response:** [`investments-v2-module-completion-fe-closure.md`](./investments-v2-module-completion-fe-closure.md)  
**Earlier FE closure (BE-SCAN batch):** [`investments-v2-be-scan-answers-fe-closure.md`](./investments-v2-be-scan-answers-fe-closure.md)

Historical: `investments-v2-be-scan-answers.md`, `investments-v2-e2e-be-answers.md`, `investments-v2-fe-gap-closure.md`  
Also: [`investments-v2-backend-asks.md`](./investments-v2-backend-asks.md) · [`investments-v2-end-to-end-gaps.md`](./investments-v2-end-to-end-gaps.md) · [`investments-v2-backend-asks-scan.md`](./investments-v2-backend-asks-scan.md)
...
```text
A1 seed/migrate: DONE | npm run db:seed:investments-v2-t0  (nts only; asserts DEV DB)
...
A6 send-to-broker body: empty OK | required: Idempotency-Key + expectedVersion; defaults venueCode/brokerProfileId
...
A17 order→trade UAT path: APPROVED → POST /orders/:id/execute (Idempotency-Key + expectedVersion)
...
| A6 | ALREADY | Orderbook send uses `expectedVersion` + Idempotency-Key |
...
npm run db:seed:investments-v2-t0
```

Leaves nonempty: portfolios, ACTIVE DLTA + `listedEquitySecurityId`, approved mark, brokers/custodians, cash accounts + providers + layouts, cash account on Arcus Listed Portfolio fund id.

Transient remote MySQL (`P1001`/`P1017`): demo seed retries valuation snapshot writes; T0 wrapper retries `db:seed:investment-ops-demo` once.
...
| F5 | `/orders/orderbook` | **Wired** — Execute + Reject (+ existing submit/approve/send/cancel). Add-to-blotter still optional |
| F6 | `/orders/blotter` | Partial — Confirm/Settle live; routing hops / settlement-doc optional polish |
| F7 | `/accounting` | **Wired** — submit → approve → post; `POST /accounting/ledger-exports` + download |
| F8 | `/valuation` exceptions | **Wired** — escalate / approve-override / reject-override (+ resolve/override) |
| F9 | `/reconciliation/exceptions` | **Wired** — file picker → upload pipeline → attach `{fileId}` |
...
2. `POST /orders/:id/execute` with `Idempotency-Key` + `expectedVersion`  
...
1. FE runs T0 checklist against seeded `nts`  
2. FE closes Part B F1–F13 — **mostly done**; F6/F12/F13 optional  
3. Hide P4 Export buttons — **done**  
4. Mark IDs closed in this file — **done**; avoid a third parallel asks doc  
