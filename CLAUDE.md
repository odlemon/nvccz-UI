...
- **No instructional captions in the UI**: don't add subtitle/caption text that just narrates how to use the control right below it (e.g. "Use the three-dot menu for actions. Clicking a UID opens the preview."). Users don't want a running tutorial under every table; a caption should carry real content (what the data is, a constraint that matters) or not exist. Remove these when you find them in vendored "-mock" pages instead of leaving them.
