...
# The compose files pass these two as build args, but without a matching ARG
# here Docker discards them silently, so lib/portal/config.ts never saw a value
# and the vendor/events cross-portal redirects stayed disabled — that file gates
# them on the variable being present at all.
ARG NEXT_PUBLIC_VENDOR_PORTAL_URL
ARG NEXT_PUBLIC_EVENTS_PORTAL_URL
ARG NEXT_PUBLIC_APPLY_PORTAL_URL
...
ENV NEXT_PUBLIC_VENDOR_PORTAL_URL=$NEXT_PUBLIC_VENDOR_PORTAL_URL
ENV NEXT_PUBLIC_EVENTS_PORTAL_URL=$NEXT_PUBLIC_EVENTS_PORTAL_URL
ENV NEXT_PUBLIC_APPLY_PORTAL_URL=$NEXT_PUBLIC_APPLY_PORTAL_URL
