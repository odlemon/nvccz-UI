...
try {
  ...
  console.log("\n== the department head approves A and B from the Approval Centre")
  {
    ...
    check(/Pending Head of Operations/.test(await rowA.first().innerText()), "A's status names the step it waits on", (await rowA.first().innerText()).replace(/\s+/g, " ").slice(0, 120))
