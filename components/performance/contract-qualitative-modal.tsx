...
interface ContractQualitativeModalProps {
  isOpen: boolean
  onClose: () => void
  type: "CEO" | "BOARD"
  periodLabel: string
...
export function ContractQualitativeModal({
  isOpen,
  onClose,
  type,
  periodLabel,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (type === "CEO") {
        await scorecardApiService.saveCeoQualitativeEvaluation(payload)
      } else {
        await scorecardApiService.saveBoardQualitativeEvaluation(payload)
      }
      toast.success(`${type} qualitative evaluation saved successfully`)
      onSaved()
      onClose()
    } catch (err: any) {
  ...
  const subjectLabel = type === "CEO" ? "Chief Executive Officer" : "Board of Directors"
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="!w-[95vw] sm:!max-w-[1200px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Rate the {subjectLabel}&apos;s leadership across 10 personal attributes for{" "}
