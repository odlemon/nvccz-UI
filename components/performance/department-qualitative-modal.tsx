...
interface DepartmentQualitativeModalProps {
  isOpen: boolean
  onClose: () => void
...
export function DepartmentQualitativeModal({
  isOpen,
  onClose,
  departmentName,
  periodLabel,
  existingAttributes,
  onSaved,
}: DepartmentQualitativeModalProps) {
  ...
  const handleSave = async () => {
    setSaving(true)
    try {
      await scorecardApiService.saveDepartmentQualitativeEvaluation(departmentName, {
        periodLabel,
        qualitativeEvaluation: { personalAttributes: attributes },
      })
      toast.success("Qualitative evaluation saved successfully")
      onSaved()
      onClose()
    } catch (err: any) {
    ...
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
      <DialogContent className="!w-[95vw] sm:!max-w-[1200px] max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Rate the Head of Department's leadership across 10 personal attributes for{" "}
            <span className="font-medium">{departmentName}</span> — {periodLabel}.
            Select one rating per attribute.
          </DialogDescription>
        </DialogHeader>
