...
export function startPortfolioV11Runtime(rootEl, options = {}) {
  ...
  const LP_ALLOCATION_COLORS=['#2475f5','#0ba780','#f5a623','#dc3f72','#0c879f','#7c4dff'];
  function lpAllocations(lp) {
    const breakdown=(lp&&lp.fundBreakdown)||[];
    if(!breakdown.length) return [{label:'No commitments yet',value:1,color:'#d0d5dd',display:'No commitments on file'}];
    const total=breakdown.reduce((sum,f)=>sum+f.commitment,0)||1;
    return breakdown.map((f,index)=>({
      label:f.fundName,
      value:f.commitment,
      color:LP_ALLOCATION_COLORS[index%LP_ALLOCATION_COLORS.length],
      display:`${formatMoney(f.commitment)} (${Math.round(f.commitment/total*100)}%)`
    }));
  }
  ...
  function renderLPOverviewTab(lp) {
    const allocations=lpAllocations(lp);
    const fundRows=(lp.fundBreakdown||[]);
    return `<section class="profile-tab-panel split-layout section-gap"><div>
      <section class="grid cols-3">
      ...
      <section class="grid cols-2 section-gap">
        ${card('Performance by Fund',`<div class="table-wrap"><table class="criteria-table"><thead><tr><th>Fund</th><th class="text-right">Commitment</th><th class="text-right">Called</th><th class="text-right">Distributed</th><th class="text-right">DPI</th></tr></thead><tbody>${fundRows.length?fundRows.map(f=>`<tr class="clickable" data-action="chart-drilldown" data-chart-label="${escapeHTML(f.fundName)}" data-chart-value="LP performance by fund"><td class="table-primary brand-text">${escapeHTML(f.fundName)}</td><td class="text-right">${formatMoney(f.commitment)}</td><td class="text-right">${formatMoney(f.called)}</td><td class="text-right">${formatMoney(f.distributed)}</td><td class="text-right">${f.dpi!=null?f.dpi.toFixed(2)+'x':'—'}</td></tr>`).join(''):'<tr><td colspan="5" class="muted">No fund commitments on file.</td></tr>'}${fundRows.length?`<tr><td class="table-primary">Total</td><td class="text-right table-primary">${formatMoney(lp.commitment)}</td><td class="text-right table-primary">${formatMoney(lp.called)}</td><td class="text-right table-primary">${formatMoney(lp.distributed)}</td><td class="text-right table-primary">${lp.called>0?(lp.distributed/lp.called).toFixed(2)+'x':'—'}</td></tr>`:''}</tbody></table></div>`) }
      ...
      ${card('Portal Access & Permissions',`<div class="info-list"><div class="info-row"><span>Portal Status</span><strong class="${lp.portal==='Active'?'positive':'muted'}">${escapeHTML(lp.portal)}</strong></div><div class="info-row"><span>Users</span><strong>${lp.portalUserCount||0}</strong></div><div class="info-row"><span>Roles</span><strong>${lp.portalRoles?`Viewer (${lp.portalRoles.viewer}), Manager (${lp.portalRoles.manager})`:'—'}</strong></div></div>`,{footer:'<button type="button" class="card-link" data-action="lp-profile-tab" data-tab="portal">Manage access</button>'})}
  ...
  function renderLPCommitmentsTab(lp) {
    ...
    return `<section class="profile-tab-panel section-gap">
      ...
      <section class="grid cols-3 section-gap">
        ...
        ${card('Called vs Unfunded',fundRows.length?barChart({labels:fundRows.map(f=>f.fundName.replace('Matanho ','')),series:[{name:'Called',color:'var(--blue)',values:fundRows.map(f=>f.called)},{name:'Unfunded',color:'var(--amber)',values:fundRows.map(f=>f.unfunded)}],height:295,yLabel:'USD',stacked:true,format:v=>formatMoney(v)}):'<p class="muted">No commitments on file.</p>',{subtitle:'Select a fund for commitment detail'})}
        ${card('Commitment Concentration',donutChart(allocations,String(fundRows.length),'Funds',150),{subtitle:'Exposure by fund'})}
      </section>
      ${card('Commitment Register',`<div class="table-wrap"><table><thead><tr><th>Fund</th><th>Vintage</th><th>Commitment Date</th><th class="text-right">Commitment</th><th class="text-right">Called</th><th class="text-right">Unfunded</th><th>Status</th></tr></thead><tbody>${fundRows.length?fundRows.map(f=>`<tr class="clickable" data-action="chart-drilldown" data-chart-label="${escapeHTML(f.fundName)}" data-chart-value="${formatMoney(f.commitment)} commitment"><td class="table-primary brand-text">${escapeHTML(f.fundName)}</td><td>${f.vintage||'—'}</td><td>${f.commitmentDate}</td><td class="text-right">${formatMoney(f.commitment)}</td><td class="text-right">${formatMoney(f.called)}</td><td class="text-right">${formatMoney(f.unfunded)}</td><td>${statusPill(f.status)}</td></tr>`).join(''):'<tr><td colspan="7" class="muted">No commitments on file.</td></tr>'}</tbody></table></div>`) }
      <section class="grid cols-2 section-gap">
        ${card('Unfunded Forecast',lineChart({labels:['Q3 2026','Q4 2026','Q1 2027','Q2 2027','Q3 2027','Q4 2027'],series:[{name:'Base case',color:'var(--blue)',values:[95,82,73,63,51,43]},{name:'High deployment',color:'var(--red)',values:[95,76,61,48,34,23]}],height:280,yLabel:'USD millions',format:v=>`${Math.round(v)}M`}),{subtitle:'Illustrative — not yet tracked per LP'})}
        ${card('Side Letter & Special Terms',`<p class="muted">No side letters or special terms on file.</p>`) }
      </section>
    </section>`;
  }

  function renderLPCapitalTab(lp) {
    const activity=(lp.transactions||[]);
    const calledTotal=activity.filter(t=>t.type==='Capital Call').reduce((s,t)=>s+t.amount,0);
    const distributedTotal=activity.filter(t=>t.type==='Distribution').reduce((s,t)=>s+t.amount,0);
    const chronological=activity.slice().sort((a,b)=>new Date(a.date)-new Date(b.date));
    let runningCalled=0,runningDistributed=0;
    const movementLabels=[],movementCalled=[],movementDistributed=[];
    chronological.forEach(t=>{
      if(t.type==='Capital Call') runningCalled+=t.amount; else runningDistributed+=t.amount;
      movementLabels.push(t.date);
      movementCalled.push(runningCalled/1000000);
      movementDistributed.push(runningDistributed/1000000);
    });
    const lastPaid=activity.filter(t=>t.paid>0).slice().sort((a,b)=>new Date(b.date)-new Date(a.date))[0];
    return `<section class="profile-tab-panel section-gap">
      ...
      <section class="grid cols-3 section-gap">
        ${card('Capital Account Movement',movementLabels.length>1?lineChart({labels:movementLabels,series:[{name:'Cumulative called',color:'var(--blue)',values:movementCalled},{name:'Cumulative distributed',color:'var(--emerald)',values:movementDistributed}],height:300,yLabel:'USD millions',format:v=>`${Number(v).toFixed(1)}M`}):`<p class="muted">Not enough recorded activity to chart a trend yet.</p>`,{subtitle:'Running total from recorded capital calls and distributions'})}
        ${card('Activity Mix',(calledTotal+distributedTotal)>0?donutChart([{label:'Capital calls',value:calledTotal,color:'#2475f5',display:formatMoney(calledTotal)},{label:'Distributions',value:distributedTotal,color:'#0ba780',display:formatMoney(distributedTotal)}],formatMoney(calledTotal+distributedTotal),'Total flows',155):`<p class="muted">No capital activity on file.</p>`)}
        ${card('Upcoming Liquidity',barChart({labels:['Aug 2026','Sep 2026','Oct 2026','Nov 2026','Dec 2026'],series:[{name:'Expected calls',color:'var(--blue)',values:[15,0,12,6,8]},{name:'Expected distributions',color:'var(--emerald)',values:[0,4,0,7,3]}],height:300,yLabel:'USD millions',format:v=>`${Math.round(v)}M`}),{subtitle:'Illustrative — not yet tracked per LP'})}
      </section>
      ${card('Transaction Ledger',`<div class="table-wrap"><table><thead><tr><th>Date</th><th>Type</th><th>Fund</th><th class="text-right">Amount</th><th class="text-right">Paid</th><th>Status</th></tr></thead><tbody>${activity.length?activity.map(row=>`<tr><td>${row.date}</td><td class="table-primary">${row.type}</td><td>${escapeHTML(row.fundName)}</td><td class="text-right">${formatMoney(row.amount)}</td><td class="text-right">${formatMoney(row.paid)}</td><td>${statusPill(row.status)}</td></tr>`).join(''):'<tr><td colspan="6" class="muted">No capital calls or distributions on file for this LP.</td></tr>'}</tbody></table></div>`) }
      <section class="grid cols-2 section-gap">
        ${card('Payment & Reconciliation',`<div class="info-list">${[['Last payment received',lastPaid?`${lastPaid.date} · ${formatMoney(lastPaid.paid)}`:'No payments on file'],['Bank account','Not on file'],['Unreconciled receipts','Not tracked'],['Withholding tax documents','Not on file']].map(row=>`<div class="info-row"><span>${row[0]}</span><strong>${row[1]}</strong></div>`).join('')}</div>`) }
        ${card('Collection Timeliness',barChart({labels:['Call 01','Call 02','Call 03','Call 04','Call 05','Call 06'],series:[{name:'Days before due date',color:'var(--emerald)',values:[8,5,12,3,7,10]}],height:260,yLabel:'Days',format:v=>`${Math.round(v)} days`}),{subtitle:'Illustrative — not yet tracked per LP'})}
      </section>
    </section>`;
  }
  ...
  function renderLPPerformanceTab(lp) {
    ...
    return `<section class="profile-tab-panel section-gap">
      ...
      <section class="grid cols-3 section-gap">
        ${card('Net IRR Trend',lineChart({labels:['Q2 2024','Q3 2024','Q4 2024','Q1 2025','Q2 2025','Q3 2025','Q4 2025','Q1 2026','Q2 2026'],series:[{name:'LP Net IRR',color:'var(--blue)',values:[7.1,8.4,9.3,10.1,11.0,11.9,12.8,13.6,14.2]},{name:'Private markets benchmark',color:'var(--emerald)',values:[6.2,7.1,7.8,8.5,9.2,9.8,10.5,11.1,11.8]}],height:300,yLabel:'Net IRR %',format:v=>`${Number(v).toFixed(1)}%`}),{subtitle:'Illustrative — not yet tracked per LP'})}
        ${card('TVPI & DPI Progression',lineChart({labels:['Q2 2024','Q4 2024','Q2 2025','Q4 2025','Q2 2026'],series:[{name:'TVPI',color:'var(--brand)',values:[1.18,1.29,1.43,1.62,1.84]},{name:'DPI',color:'var(--emerald)',values:[.08,.13,.19,.25,.31]}],height:300,yLabel:'Multiple (x)',format:v=>`${Number(v).toFixed(2)}x`}),{subtitle:'Illustrative — not yet tracked per LP'})}
        ${card('Return Attribution',donutChart(allocations,pct(lp.netIrr),'Net IRR',155),{subtitle:'Commitment share by fund'})}
      </section>
      ${card('Performance by Fund',`<div class="table-wrap"><table><thead><tr><th>Fund</th><th>Vintage</th><th class="text-right">Commitment</th><th class="text-right">Distributed</th><th class="text-right">DPI</th></tr></thead><tbody>${fundRows.length?fundRows.map(f=>`<tr class="clickable" data-action="chart-drilldown" data-chart-label="${escapeHTML(f.fundName)}" data-chart-value="LP fund performance"><td class="table-primary brand-text">${escapeHTML(f.fundName)}</td><td>${f.vintage||'—'}</td><td class="text-right">${formatMoney(f.commitment)}</td><td class="text-right">${formatMoney(f.distributed)}</td><td class="text-right">${f.dpi!=null?f.dpi.toFixed(2)+'x':'—'}</td></tr>`).join(''):'<tr><td colspan="5" class="muted">No fund commitments on file.</td></tr>'}</tbody></table></div>`) }
      <section class="grid cols-2 section-gap">
  ...
  function renderLPDocumentsTab(lp) {
    const rows=(lp.documents||[]);
    const categoryCount=new Set(rows.map(d=>d.category)).size;
    const latest=rows.slice().sort((a,b)=>new Date(b.publishedAt||0)-new Date(a.publishedAt||0))[0];
    return `<section class="profile-tab-panel section-gap">
      ...
      <section class="grid cols-4 section-gap">
        ${metricCard({label:'Documents',value:String(rows.length),iconName:'file',accent:'blue',foot:'All published LP documents',action:'lp-documents'})}
        ${metricCard({label:'KYC Status',value:lp.kyc,iconName:'user-check',accent:'emerald',foot:'From client record',action:'lp-kyc'})}
        ${metricCard({label:'Categories',value:String(categoryCount),iconName:'folder',accent:'amber',foot:categoryCount?'Distinct document categories':'No documents on file',action:'lp-documents'})}
        ${metricCard({label:'Latest Upload',value:latest&&latest.publishedAt?latest.publishedAt:'—',iconName:'clock',accent:'purple',foot:latest?escapeHTML(latest.title):'No documents on file',action:'lp-documents'})}
      </section>
      ${card('Document Register',`<div class="table-toolbar"><div class="table-search">${icon('search')}<input type="text" placeholder="Search LP documents..."></div><div class="table-tools">${button('Filter','lp-filters','compact','filter')}${button('Export index','export-lps','compact','download')}</div></div><div class="table-wrap"><table><thead><tr><th>Document</th><th>Category</th><th>Fund</th><th>Published</th><th>Type</th><th></th></tr></thead><tbody>${rows.length?rows.map(row=>`<tr class="clickable" data-action="preview-document" data-id="${row.id}"><td class="table-primary"><button type="button" class="v17-document-name" data-action="preview-document" data-id="${row.id}">${icon('file')}<span>${escapeHTML(row.title)}</span></button></td><td>${escapeHTML(row.category)}</td><td>${escapeHTML(row.fundName)}</td><td>${row.publishedAt?row.publishedAt:'Not published'}</td><td>${row.mimeType||'—'}</td><td><div class="document-row-actions"><button type="button" class="button ghost compact icon-only" data-action="preview-document" data-id="${row.id}">${icon('eye')}</button><button type="button" class="button ghost compact icon-only" data-action="download-document" data-id="${row.id}">${icon('download')}</button></div></td></tr>`).join(''):'<tr><td colspan="6" class="muted">No documents on file for this LP.</td></tr>'}</tbody></table></div>`) }
      <section class="grid cols-2 section-gap">
        ${card('KYC Review Checklist',`<p class="muted">Illustrative — not yet tracked per LP.</p>`) }
        ${card('Open Requests',`<p class="muted">Illustrative — not yet tracked per LP.</p>`) }
      </section>
    </section>`;
  }
  ...
  function renderLPPortalTab(lp) {
    ...
    return `<section class="profile-tab-panel section-gap">
      ...
      <section class="grid cols-2 section-gap">
        ${metricCard({label:'Portal Status',value:lp.portal,iconName:'shield',accent:lp.portal==='Active'?'emerald':'amber',foot:lp.portal==='Active'?'Has portal access':'No portal account linked yet',action:'lp-portal-status'})}
        ${metricCard({label:'Active Users',value:String(lp.portalUserCount||0),iconName:'users',accent:'blue',foot:`${lp.portalRoles.viewer} viewer(s) · ${lp.portalRoles.manager} manager(s)`,action:'lp-portal-users'})}
      </section>
      ${card('Portal Users',`<div class="table-wrap"><table><thead><tr><th>User</th><th>Role</th><th>Email</th><th>Status</th><th></th></tr></thead><tbody>${users.length?users.map(u=>`<tr class="clickable" data-action="manage-lp-access"><td class="table-primary">${escapeHTML(u.name)}</td><td>${escapeHTML(u.role)}</td><td>${escapeHTML(u.email)}</td><td>${statusPill(u.isActive?'Active':'Revoked')}</td><td><button type="button" class="button ghost compact icon-only" data-action="activity-menu" data-context="lp" data-id="${lp.id}" aria-label="Portal user activity">${icon('clock')}</button></td></tr>`).join(''):'<tr><td colspan="5" class="muted">No portal users invited yet.</td></tr>'}</tbody></table></div>`) }
      <section class="grid cols-2 section-gap">
        ${card('Permission Matrix',`<div class="table-wrap"><table class="permission-matrix"><thead><tr><th>Capability</th><th>Manager</th><th>Viewer</th></tr></thead><tbody>${[['View fund performance','✓','✓'],['Download reports','✓','✓'],['Acknowledge notices','✓','—'],['Manage users','✓','—'],['Update contact details','✓','—'],['View KYC records','✓','—'],['Upload documents','✓','—']].map(row=>`<tr>${row.map((cell,i)=>`<td class="${i===0?'table-primary':''}">${cell}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`,{footer:'<button type="button" class="card-link" data-action="manage-lp-access">Edit permission policy</button>'})}
        ${card('Security & Session Controls',`<p class="muted">Session and audit-log tracking is not yet available per LP.</p>`,{footer:'<button type="button" class="card-link" data-action="security-settings">Open security settings</button>'})}
      </section>
    </section>`;
  }
