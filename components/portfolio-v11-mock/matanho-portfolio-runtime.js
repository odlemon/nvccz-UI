...
export function startPortfolioV11Runtime(rootEl, options = {}) {
  ...
  function renderFundPerformance() {
    ...
    const metrics = [
      {label:'Gross IRR',value:pct(selectedFund.grossIrr),iconName:'trend-up',accent:'emerald',foot:'1.6pp vs Q1 2026',action:'metric-gross-irr'},
      {label:'Net IRR',value:pct(selectedFund.netIrr),iconName:'users',accent:'cyan',foot:'1.3pp vs Q1 2026',action:'metric-net-irr'},
      {label:'TVPI',value:`${selectedFund.tvpi.toFixed(2)}x`,iconName:'bar-chart',accent:'amber',foot:'0.14x vs Q1 2026',action:'metric-tvpi'},
      {label:'DPI',value:`${selectedFund.dpi.toFixed(2)}x`,iconName:'dollar',accent:'purple',foot:'0.08x vs Q1 2026',action:'metric-dpi'},
      {label:'RVPI',value:`${Math.max(0,selectedFund.tvpi-selectedFund.dpi).toFixed(2)}x`,iconName:'trend-up',accent:'blue',foot:'0.06x vs Q1 2026',action:'metric-rvpi'},
      {label:'NAV',value:formatMoney(selectedFund.nav),iconName:'dollar',accent:'emerald',foot:'+8.7M vs Q1 2026',action:'metric-nav'}
    ];
  ...
  renderFundPerformance = function() {
    ...
    const metricSets = {
      Performance:[['Gross IRR',pct(selectedFund.grossIrr),'trend-up','emerald','+1.6pp vs prior quarter'],['Net IRR',pct(selectedFund.netIrr),'users','cyan','+1.3pp vs prior quarter'],['TVPI',`${selectedFund.tvpi.toFixed(2)}x`,'bar-chart','amber','+0.14x vs prior quarter'],['DPI',`${selectedFund.dpi.toFixed(2)}x`,'dollar','purple','+0.08x vs prior quarter'],['RVPI',`${Math.max(0,selectedFund.tvpi-selectedFund.dpi).toFixed(2)}x`,'trend-up','blue','Residual value multiple'],['NAV',formatMoney(selectedFund.nav),'dollar','emerald','Approved period close']],
      ...
      Portfolio:[['Portfolio companies',String(Math.max(5,companies.filter(c=>c.fund===selectedFund.name).length)),'building','blue','Active investments'],['Fair value',formatMoney(selectedFund.nav),'dollar','emerald','Approved valuation'],['Weighted revenue growth','28.9%','trend-up','cyan','Current quarter'],['Weighted EBITDA growth','22.7%','bar-chart','purple','Current quarter'],['On-track companies','82%','check-circle','emerald','Weighted by fair value'],['Watch items','3','alert','amber','Require intervention']],
