      ...
      <div className="p-4 sm:p-5 space-y-4">
        ...
        {loadingModels && models.length === 0 ? (
          ...
          <div className="rounded-xl border border-dashed border-[#cbd5e1] bg-white p-10 text-center max-w-lg mx-auto shadow-sm">
            <p className="text-sm font-medium text-[#000000]">No planning models yet</p>
          ...
          <div className="rounded-xl border border-[#e2e8f0] bg-white shadow-[0_1px_2px_rgba(15,23,42,0.04)] overflow-hidden">
            <div className="flex flex-wrap items-center gap-3 px-4 py-3 border-b border-[#e2e8f0] bg-[#fafbfc]">
              <div className="relative flex-1 min-w-[200px] max-w-sm">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-[#141414]" />
                <input
                  value={q}
                  onChange={(e) => setQ(e.target.value)}
                  placeholder="Filter by name, status, type…"
                  className="h-9 w-full rounded-full border border-[#e2e8f0] bg-white pl-9 pr-3 text-xs text-[#000000] placeholder:text-[#141414] focus:outline-none focus:ring-2 focus:ring-[#2563eb]/25"
                />
              </div>
              <p className="text-[11px] text-[#141414] ml-auto">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[960px]">
                ...
                <tbody>
                  {filtered.length === 0 ? (
                    <tr>
                      <td colSpan={8} className="px-4 py-12 text-center text-xs text-[#141414]">
                        No models match “{q}”.
                      </td>
                    </tr>
                  ) : (
                    filtered.map((m, i) => (
                      ...
                      >
                        <td className="px-4 py-3.5">
                          <span className="font-medium text-[#000000]">{m.name}</span>
                        </td>
                        <td className="px-4 py-3.5">
                        ...
                        <td className="px-4 py-3.5 text-[#111111] tabular-nums">{m.baseCurrency}</td>
