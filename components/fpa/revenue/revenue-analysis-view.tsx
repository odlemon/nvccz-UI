...
export function RevenueAnalysisView({
  ...
  if (loading && !hasData) return <div className="min-h-full bg-[#f1f5f9] flex items-center justify-center gap-2 text-sm text-[#111111]"><Loader2 className="size-5 animate-spin" /> Loading revenue analysis…</div>
      ...
      <div className="bg-white border-b border-[#e4e7ec]">
        <div className="px-4 sm:px-5 pt-4 pb-3">
          <div className="flex items-center justify-between gap-3">
            <h1 className="text-[18px] font-semibold text-[#000000]">Revenue</h1>
        ...
        <div className="px-4 sm:px-5 pb-4">
          {adjustedKpis.length === 0 ? (
            <p className="py-8 text-sm text-[#111111]">No revenue KPIs are available.</p>
          ) : (
        ...
        <div className="flex-1 p-4 sm:p-5 space-y-4 overflow-auto">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
            <section className={`${R} border border-[#e4e7ec] bg-white p-4`}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-2">
                  <h2 className="text-sm font-semibold text-[#000000]">Revenue Waterfall</h2>
                  <button type="button" onClick={() => setInfoOpen(true)} className="text-[#141414] hover:text-[#111111]">
                    <Info className="size-4" />
                  </button>
                </div>
                <span className="text-xs text-[#111111]">{period} · Budget → Forecast</span>
              </div>
              {waterfallData.length === 0 ? (
                <p className="py-12 text-center text-sm text-[#111111]">No revenue waterfall data is available.</p>
            ...
            <section className={`${R} border border-[#e4e7ec] bg-white p-4`}>
              <div className="flex items-center justify-between mb-3">
                <h2 className="text-sm font-semibold text-[#000000]">Monthly Trend</h2>
                <span className="text-xs text-[#111111]">Actual vs Budget vs Forecast</span>
              </div>
              {displayedMonthly.length === 0 ? (
                <p className="py-12 text-center text-sm text-[#111111]">No monthly revenue data is available.</p>
              ) : <div className="h-[240px]">
                <ResponsiveContainer width="100%" height="100%">
          ...
          <section className={`${R} border border-[#e4e7ec] bg-white overflow-hidden`}>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm min-w-[720px]">
                ...
                <tbody>
                  {tableRows.length === 0 ? (
                    <tr><td colSpan={8} className="px-4 py-10 text-center text-sm text-[#111111]">No revenue stream data is available.</td></tr>
                  ) : null}
                  {tableRows.map((row) => {
                    const varB = row.actual == null || row.budget == null ? null : row.actual - row.budget
                    return (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-[#111111]">{row.method ?? "—"}</td>
                        ...
                        <td className={cn("px-4 py-3 text-right tabular-nums", row.yoy == null ? "text-[#111111]" : varTone(row.yoy))}>
                          {row.yoy == null ? "—" : `${row.yoy >= 0 ? "+" : ""}${row.yoy.toFixed(1)}%`}
                        </td>
                        <td className="px-4 py-3 text-right tabular-nums text-[#111111]">{row.share == null ? "—" : `${row.share}%`}</td>
                      </tr>
                    )
                  })}
                </tbody>
          ...
          <section className={`${R} border border-[#e4e7ec] bg-white p-4`}>
            <div className="flex items-center gap-2 mb-4">
              ...
              <h2 className="text-sm font-semibold text-[#000000]">What-if: Revenue Growth Rate</h2>
              <span className="text-xs text-[#111111] ml-auto tabular-nums">{growthRate.toFixed(1)}%</span>
            </div>
            {drivers.length === 0 ? <p className="text-sm text-[#111111]">No revenue driver was returned, so this control is unavailable.</p> : <>
        ...
        {detailOpen && selectedDetail ? (
          <aside className="w-full sm:w-[360px] shrink-0 border-l border-[#e4e7ec] bg-white flex flex-col">
            <div className="px-4 py-3 border-b border-[#e4e7ec] flex items-start justify-between gap-2">
              <div>
                <p className="text-xs text-[#111111]">{selectedDetail.region ?? "—"} · {selectedDetail.period}</p>
                <h3 className="text-base font-semibold text-[#000000] mt-0.5">{selectedDetail.name}</h3>
                <p className="text-xs text-[#141414] mt-1">{selectedDetail.method ?? "—"}</p>
              </div>
            ...
            <div className="p-4 space-y-4 overflow-auto flex-1">
              <div className={`${R} bg-[#f9fafb] border border-[#e4e7ec] p-3`}>
                ...
                <p className={cn("text-2xl font-semibold tabular-nums mt-1", selectedDetail.variance === "—" ? "text-[#111111]" : selectedDetail.variance.startsWith("+") ? "text-[#12b76a]" : "text-[#f04438]")}>
                  {selectedDetail.variance}
                </p>
                <p className="text-xs text-[#111111] mt-1">YoY {selectedDetail.yoy}</p>
              </div>
              ...
              {selectedDetail.drivers.length > 0 ? <div>
                <p className="text-xs font-semibold text-[#111111] mb-2">Key Drivers</p>
              ...
              </div> : <p className="text-sm text-[#111111]">Driver details and commentary are not included in this domain response.</p>}
            </div>
          </aside>
        ) : null}
      </div>
      ...
      {infoOpen ? (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30 p-4" onClick={() => setInfoOpen(false)}>
          <div className={`${R} bg-white max-w-md w-full p-5 shadow-xl`} onClick={(e) => e.stopPropagation()}>
            <h3 className="text-base font-semibold text-[#000000]">Revenue Analysis</h3>
