    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl rounded-2xl border border-[#e2e8f0] p-0 gap-0 overflow-hidden max-h-[85vh] flex flex-col">
        <DialogHeader className="px-5 pt-5 pb-3 border-b border-[#f1f5f9] shrink-0">
          <DialogTitle className="text-base text-[#000000]">Review queue</DialogTitle>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-5">
          {empty ? (
            <div className="py-10 text-center">
              <p className="text-sm font-medium text-[#111111]">No items in the review queue</p>
              <p className="text-xs text-[#141414] mt-1.5 max-w-sm mx-auto">
  ...
  return (
    <section>
      <div className="flex items-center justify-between mb-2.5">
        ...
        <span className="text-[11px] tabular-nums text-[#141414]">{count}</span>
      </div>
      {tasks.length === 0 ? (
        <p className="text-[12px] text-[#141414] py-3 px-3 rounded-lg bg-[#f8fafc]">{emptyLabel}</p>
      ) : (
            ...
            <li key={t.id}>
              ...
              >
                <div className="min-w-0 flex-1">
                  <p className="text-[13px] font-medium text-[#000000] truncate">{t.title}</p>
                ...
                <div className="shrink-0 text-right space-y-1">
                  ...
                  <p className="text-[11px] text-[#141414] tabular-nums">
                    {t.dueDate
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-xl rounded-2xl border border-[#e2e8f0] p-0 gap-0 overflow-hidden max-h-[85vh] flex flex-col">
        <DialogHeader className="px-5 pt-5 pb-3 border-b border-[#f1f5f9] shrink-0">
          <DialogTitle className="text-base text-[#000000]">Recent approvals</DialogTitle>
          <DialogDescription className="text-sm text-[#111111]">
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4">
          {rows.length === 0 ? (
            <div className="py-10 text-center">
              <p className="text-sm font-medium text-[#111111]">No approvals yet</p>
              <p className="text-xs text-[#141414] mt-1.5 max-w-sm mx-auto">
